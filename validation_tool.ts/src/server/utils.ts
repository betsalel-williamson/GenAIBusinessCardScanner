...
export const getFileStatus = async (
  jsonFilename: string,
): Promise<"validated" | "in_progress" | "source"> => {
  ...
  try {
    ...
    /* Not in source either, might be an old file reference or error */
    console.error(e);
  }
...
export const loadData = async (
  jsonFilename: string,
): Promise<DataRecord | DataRecord[] | null> => {
  const inProgressPath = path.join(IN_PROGRESS_DATA_DIR, jsonFilename);
  const validatedPath = path.join(VALIDATED_DATA_DIR, jsonFilename);
  const sourcePath = path.join(SOURCE_DATA_DIR, jsonFilename);
