...
export const useValidationData = (dataEntryPaneRef: React.RefObject<DataEntryPaneHandle>): UseValidationDataHook => {
    ...
    useEffect(() => {
        ...
        fetch(`/api/files/${json_filename}`)
            ...
            .then((data: DataRecord) => { // Expect a single DataRecord
                ...
                if (Array.isArray(data)) {
                    if (data.length > 0) {
                        console.warn("Received an array for single record. Using the first element.");
                        resetRecord(data[0]);
                    } else {
                    ...
                    throw new Error("Invalid data format: Expected a single record object.");
                }
    ...
    const handleAddField = useCallback((key: string, value: string) => {
        if (!currentRecord) return;
        if (!(key in currentRecord)) { // Only add if field doesn't exist
    ...
    const handleCommit = useCallback(async () => {
        ...
        if (!window.confirm("Commit this record and move to the next unvalidated file?")) {
            return;
        }
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if ((event.metaKey || event.ctrlKey) && event.key === 'z') {
                if (canUndoRecord) { // Check against renamed state
            ...
            if ((event.metaKey || event.ctrlKey) && (event.key === 'y' || (event.shiftKey && event.key === 'Z'))) {
                if (canRedoRecord) { // Check against renamed state
