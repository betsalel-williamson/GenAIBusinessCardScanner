...
| **Milestone** | **`backlog task edit <id> --set milestone="<milestone_name>"`** |
...
Milestones are used to group tasks into larger releases or phases. They are defined in the `milestones` array within the `backlog/config.yml` file. Once defined, tasks can be associated with a milestone using the `backlog task edit` command with the `--set` flag.

**Example:** To assign task `123` to the `v1.0` milestone:
`backlog task edit 123 --set milestone="v1.0"`
