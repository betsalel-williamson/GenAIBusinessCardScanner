...
@patch(
    "postmarker.core.PostmarkClient.__new__"
)  # Patch __new__ to control instance creation
def test_postmark_email_client_send_templated_email(mock_postmark_client_new):
    ...
    mock_client_instance = MagicMock(
        spec=PostmarkClient
    )  # Mock the instance returned by PostmarkClient()
    mock_client_instance.emails = (
        MagicMock()
    )  # Ensure the mock instance has an 'emails' attribute
    mock_postmark_client_new.return_value = (
        mock_client_instance  # What PostmarkClient() returns
    )
    ...
    client = PostmarkEmailClient(
        api_token="test_token", sender_email="sender@example.com"
    )
    ...
    # Assert
    mock_postmark_client_new.assert_called_once_with(
        PostmarkClient, server_token="test_token"
    )  # Assert on the __new__ call
